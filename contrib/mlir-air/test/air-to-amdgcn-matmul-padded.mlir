...
// pad_tiling_interface pads 40→48 at the iteration domain level.
// After padding, all tiles are full (48 % 16 == 0). No boundary tiles.
// C output is 48x48 (padded); host extracts valid 40x40 region.
...
module {
  amdgcn.library @linalg_lib isa = [#amdgcn.isa<cdna3>] {
    ...
    // Same library functions as the 64x64 test — all tiles are full after
    // pad_tiling_interface pads 40→48.
  ...
  amdgcn.module @matmul_mod target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
    // Kernel operates on actual 40x40 dimensions.
    // pad_tiling_interface in the transform pads to 48x48 internally.
    // Host over-allocates C (48*48 elements) to fit padded stores.
    func.func @matmul_f16_40x40(
      ...
      // matmul_transpose_b on actual 40-element M/N dims.
      // pad_tiling_interface will pad these to 48 before tiling.
