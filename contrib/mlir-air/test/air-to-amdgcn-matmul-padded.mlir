// Padded matmul E2E test: actual dimensions M=40, N=40, K=64.
// Host pads inputs to tile-aligned sizes: A=48x64, B=48x64, C=48x48.
// Kernel operates on full tile-aligned dimensions; padding zeros produce
// zero output in the padded region. Host extracts valid C[0:40, 0:40].
...
module {
  ...
  amdgcn.module @matmul_mod target = #amdgcn.target<gfx942> isa = #amdgcn.isa<cdna3> {
    // 48x48x64 matmul on tile-aligned dimensions.
    // Host pads actual 40x40 data to these sizes.
    func.func @matmul_f16_48x48(
        %A: memref<48x64xf16>, %B: memref<48x64xf16>, %C: memref<48x48xf32>)
        attributes {gpu.kernel} {
      %cst = arith.constant 0.000000e+00 : f32
      %a = bufferization.to_tensor %A restrict writable : memref<48x64xf16> to tensor<48x64xf16>
      %b = bufferization.to_tensor %B restrict writable : memref<48x64xf16> to tensor<48x64xf16>
      %c = bufferization.to_tensor %C restrict writable : memref<48x48xf32> to tensor<48x48xf32>
      %fill = linalg.fill ins(%cst : f32) outs(%c : tensor<48x48xf32>) -> tensor<48x48xf32>
      ...
      } ins(%a, %b : tensor<48x64xf16>, tensor<48x64xf16>)
        outs(%fill : tensor<48x48xf32>) {
      ...
      bufferization.materialize_in_destination %result in writable %C
          : (tensor<48x48xf32>, memref<48x48xf32>) -> ()
      return
    }
  }
}
